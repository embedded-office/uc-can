...
*/
/*------------------------------------------------------------------------------------------------*/
STM32F20X_CAN_DATA DevData[STM32F20X_CAN_N_DEV];
...
{
    STM32F20X_CAN_DATA *dev;                          /* Local: Pointer to can device             */
    STM32F20X_CAN_t    *can;                          /* Local: Pointer to can register           */
    STM32F20X_CAN_t    *can0;                         /* Local: Pointer to can register           */
    CPU_INT32U          i;                            /* Local: loop variable                     */
    CPU_INT32U          tmp;                          /* Local: help variable                     */
    CPU_INT16S          result = -1;                  /* Local: Function result                   */
                                                      /*------------------------------------------*/
#if STM32F20X_CAN_ARG_CHK_CFG > 0                     /* Checking arguments (if enabled)          */
    if (arg >= STM32F20X_CAN_N_DEV) {                 /* can dev out of range?                    */
        DrvError = STM32F20X_CAN_INIT_ERR;
        return (result);
    }
#endif                                                /*------------------------------------------*/
    DrvError = STM32F20X_CAN_NO_ERR;                  /* reset to 0                               */
                                                      /*------------------------------------------*/
    dev = &DevData[arg];                              /* set pointer to can device                */
    dev->Use = 0;                                     /* set device to unused                     */
                                                      /*------------------------------------------*/
    STM32F20XCAN_PinCfg(arg);                         /* apply pin configuration                  */
                                                      /*------------------------------------------*/
    if (arg == STM32F20X_CAN_BUS_0) {
        dev->Base = (void *)STM32F20X_CAN1_BASE;          /* set CAN 1 base address               */
        *(volatile CPU_INT32U *)0x40023840 |= 0x02000000; /* RCC_APB1ENR: enable CAN 1 clock      */
    } else {
        DevData[STM32F20X_CAN_BUS_0].Base = (void *)STM32F20X_CAN1_BASE;/* set CAN 1 base address */
        *(volatile CPU_INT32U *)0x40023840 |= 0x02000000; /* RCC_APB1ENR: enable CAN 1 clock also */
        dev->Base = (void *)STM32F20X_CAN2_BASE;          /* set CAN 2 base address               */
        *(volatile CPU_INT32U *)0x40023840 |= 0x04000000; /* RCC_APB1ENR: enable CAN 2 clock      */
    }

    can = (STM32F20X_CAN_t *)dev->Base;
                                                      /*------------------------------------------*/
    can->MCR &= ~STM32F20X_CAN_MCR_SLEEP;             /* exit sleep mode                          */
                                                      /*------------------------------------------*/
    can->MCR = STM32F20X_CAN_MCR_INRQ;                /* set init bit                             */
    while ((can->MSR & STM32F20X_CAN_MSR_INAK) == 0) {/* wait for init mode                       */
    }
                                                      /*------------------------------------------*/
    dev->Baudrate        = STM32F20X_DEF_BAUDRATE;    /* Initialize parameter struct of the       */
    dev->SamplePoint     = STM32F20X_DEF_SP;          /* current device                           */
    dev->ResynchJumpWith = STM32F20X_DEF_RJW;
                                                      /*------------------------------------------*/
    STM32F20XCAN_CalcTimingReg(dev);                  /* Calculate the values for the timing      */
    ...
    can0 = (STM32F20X_CAN_t *)DevData[STM32F20X_CAN_BUS_0].Base;

    can0->FMR   |=  STM32F20X_CAN_FMR_FINIT;          /* set filter initialization to init        */
    ...
    can0->FFA1R  =  0x00000002 |                      /* can 1: fifo 0 & 1 assigned to filter  0 &  1 */
    ...
    can0->FMR   &= ~STM32F20X_CAN_FMR_FINIT;          /* set filter initialization to active      */
                                                      /*------------------------------------------*/
    can->MCR &= ~STM32F20X_CAN_MCR_INRQ;              /* Request leave initialization             */
    while ((can->MSR & STM32F20X_CAN_MSR_INAK) == 1) {/* wait for end of init mode                */
    }
                                                      /*------------------------------------------*/
    if (arg == STM32F20X_CAN_BUS_0) {
#if STM32F20X_CAN1_RX_INTERRUPT_EN > 0
        tmp = (STM32F20X_CAN1_RX1_IRQ_PRIO <<  4) +       /* shift prio to register position          */
              (STM32F20X_CAN1_RX1_IRQ_PRIO << 12);
...
#if STM32F20X_CAN1_TX_INTERRUPT_EN > 0
        tmp = (STM32F20X_CAN1_TX_IRQ_PRIO << 28);         /* shift prio to register position          */
...
#if STM32F20X_CAN1_NS_INTERRUPT_EN > 0
        tmp = (STM32F20X_CAN1_NS_IRQ_PRIO << 20);         /* shift prio to register position          */
        *(volatile CPU_INT32U *)0xE000E414 |= tmp;        /* Set priority for rx0 and rx1 irq in NVIC IPR */
...
#if STM32F20X_CAN2_RX_INTERRUPT_EN > 0
        tmp = (STM32F20X_CAN2_RX1_IRQ_PRIO <<  4) +       /* shift prio to register position          */
              (STM32F20X_CAN2_RX1_IRQ_PRIO << 12);
...
#if STM32F20X_CAN2_TX_INTERRUPT_EN > 0
        tmp = (STM32F20X_CAN2_TX_IRQ_PRIO << 28);         /* shift prio to register position          */
...
#if STM32F20X_CAN2_NS_INTERRUPT_EN > 0
        tmp = (STM32F20X_CAN2_NS_IRQ_PRIO << 20);         /* shift prio to register position          */
        *(volatile CPU_INT32U *)0xE000E440 |= tmp;        /* Set priority for rx0 and rx1 irq in NVIC IPR */
...
*/
/*------------------------------------------------------------------------------------------------*/
CPU_INT16S STM32F20XCANOpen (CPU_INT16S devId, CPU_INT32U devName, CPU_INT16U mode)
{
    STM32F20X_CAN_DATA *dev;                          /* Local: Pointer to can device             */
    CPU_INT16S          result = -1;                  /* Local: Function result                   */
    CPU_SR_ALLOC();                                   /* Allocate storage for CPU status reg.     */
                                                      /*------------------------------------------*/
#if STM32F20X_CAN_ARG_CHK_CFG > 0
    if (devName >= STM32F20X_CAN_N_DEV) {             /* check that device name is in range       */
        DrvError = STM32F20X_CAN_BUS_ERR;
        return (result);                              /* return function result                   */
    }
    if (mode != DEV_RW) {                             /* mode not supported?                      */
        DrvError = STM32F20X_CAN_MODE_ERR;
        return (result);
    }
#endif
...
#if ((STM32F20X_CAN1_RX_INTERRUPT_EN > 0) || \
     (STM32F20X_CAN1_TX_INTERRUPT_EN > 0) || \
     (STM32F20X_CAN1_NS_INTERRUPT_EN > 0) || \
     (STM32F20X_CAN2_RX_INTERRUPT_EN > 0) || \
     (STM32F20X_CAN2_TX_INTERRUPT_EN > 0) || \
     (STM32F20X_CAN2_NS_INTERRUPT_EN > 0))
        STM32F20XCAN_SetDevIds(devId, devName);       /* store the received Node Id for the irqs  */
#else
        devId    = devId;                             /* prevent compiler warning                 */
#endif
        result   = (CPU_INT16S)devName;               /* Okay, device is opened                   */
    } else {
        DrvError = STM32F20X_CAN_OPEN_ERR;
    }
    CPU_CRITICAL_EXIT();
                                                      /*------------------------------------------*/
    return (result);                                  /* return function result                   */
...
CPU_INT16S STM32F20XCANClose (CPU_INT16S paraId)
{
    ...
    CPU_SR_ALLOC();                                   /* Allocate storage for CPU status reg.     */
                                                      /*------------------------------------------*/
#if STM32F20X_CAN_ARG_CHK_CFG > 0
    if ((paraId >= STM32F20X_CAN_N_DEV) || (paraId < 0)) { /* check that paraId is in range       */
        DrvError = STM32F20X_CAN_BUS_ERR;
        return (result);                              /* return function result                   */
    }
#endif
    ...
    if (dev->Use != 0) {                              /* check, that can device is used           */
        ...
        result   = STM32F20X_CAN_NO_ERR;              /* Indicate successful function execution   */
    } else {
        DrvError = STM32F20X_CAN_CLOSE_ERR;
    }
    CPU_CRITICAL_EXIT();
                                                      /*------------------------------------------*/
    return (result);                                  /* return function result                   */
...
CPU_INT16S STM32F20XCANIoCtl (CPU_INT16S paraId, CPU_INT16U func, void *argp)
{
    ...
    switch (func) {                                   /* select: function code                    */
                                                      /*------------------------------------------*/
        case IO_STM32F20X_CAN_GET_IDENT:              /* GET IDENT                                */
            (*(CPU_INT32U *)argp) = DrvIdent;         /* return driver ident code                 */
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_GET_ERRNO:              /* GET ERRORCODE                            */
            (*(CPU_INT16U *)argp) = DrvError;         /* return last detected errorcode           */
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_GET_DRVNAME:            /* GET DRIVER NAME                          */
            (*(CPU_INT08U **)argp) = (CPU_INT08U *)   /* return human readable driver name        */
                                     STM32F20X_CAN_NAME;
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_SET_BAUDRATE:           /* SET BAUDRATE                             */
            dev->Baudrate = *(CPU_INT32U *)argp;      /* Set baudrate parameter to given argument */
                                                      /* and calculate the values for the timing  */
                                                      /* register with the parameter setting      */
            if (STM32F20XCAN_CalcTimingReg(dev) != STM32F20X_CAN_NO_ERR) {
                DrvError = STM32F20X_CAN_ARG_ERR;     /* indicate error on function result.       */
            ...
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_TX_READY:               /* TX READY?                                */
                                                      /* check for empty transmit mailbox         */
            if ((can->TSR & STM32F20X_CAN_TSR_TME0) == STM32F20X_CAN_TSR_TME0) {
                TransmitMailbox = 0;
            } else if ((can->TSR & STM32F20X_CAN_TSR_TME1) == STM32F20X_CAN_TSR_TME1) {
                TransmitMailbox = 1;
            } else if ((can->TSR & STM32F20X_CAN_TSR_TME2) == STM32F20X_CAN_TSR_TME2) {
                TransmitMailbox = 2;
            }
            ...
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_START:                  /* START CAN                                */
            can->MCR &= ~STM32F20X_CAN_MCR_INRQ;      /* reset init bit                           */
            while ((can->MSR & STM32F20X_CAN_MSR_INAK) == 1) { /* wait for end of init mode       */
            }
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_STOP:                   /* STOP CAN                                 */
            can->MCR |=  STM32F20X_CAN_MCR_INRQ;      /* set init bit                             */
            while ((can->MSR & STM32F20X_CAN_MSR_INAK) == 0) { /* wait for init mode              */
            }
            ...
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_RX_STANDARD:            /* RX STANDARD                              */
            can0->FMR |=  STM32F20X_CAN_FMR_FINIT;    /* set filter initialization to init        */
            if (paraId == STM32F20X_CAN_BUS_0) {
                for (i = 0; i < 14; i++){
            ...
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_RX_EXTENDED:            /* RX EXTENDED                              */
            can0->FMR |=  STM32F20X_CAN_FMR_FINIT;    /* set filter initialization to init        */
            if (paraId == STM32F20X_CAN_BUS_0) {
                for (i = 0; i < 14; i++){
            ...
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_GET_NODE_STATUS:        /* GET NODE STATUS                          */
            ...
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_SET_RX_FILTER_1:        /* SET RX FILTER 1                          */
            ...
            result = STM32F20X_CAN_NO_ERR;            /* Indicate successful function execution   */
            break;                                    /*------------------------------------------*/
        case IO_STM32F20X_CAN_SET_RX_FILTER_2:        /* SET RX FILTER 2                          */
            mask  = ((CPU_INT32U*)argp)[0];
            canId = ((CPU_INT32U*)argp)[1];
            can0->FMR |=  STM32F20X_CAN_FMR_FINIT;    /* set filter initialization to init        */
            if (canId > 0x7FF) {                      /* depending on the CAN ID                  */
    ...
    if (result == -1) {
        DrvError = STM32F20X_CAN_FUNC_ERR;
    }
                                                       /*------------------------------------------*/
    return (result);                                   /* Return function result                   */
}
...
CPU_INT16S STM32F20XCANRead (CPU_INT16S paraId, CPU_INT08U *buffer, CPU_INT16U size)
{
    ...
    STM32F20X_CANFRM   *frm;                          /* Local: Pointer to can frame              */
    ...
    if (size != sizeof(STM32F20X_CANFRM)) {           /* check that size is plausible             */
        DrvError = STM32F20X_CAN_NO_DATA_ERR;
        return (result);
    }
    if (buffer == (void *)0) {                        /* invalid buffer pointer                   */
        DrvError = STM32F20X_CAN_ARG_ERR;
        return (result);
    }
#endif
    dev = &DevData[paraId];                           /* set pointer to can device                */
    ...
    frm = (STM32F20X_CANFRM *)buffer;                 /* Set pointer to can frame                 */
    FIFONumber = -1;
    ...
    if (FIFONumber >= 0) {
        ...
        if (Word == 0) {                              /* is std id                                */
            ...
            frm->Identifier |= STM32F20X_FF_FRAME_BIT;/* mark ext. ID                             */
        }
        if ((can->FIFOMailBox[FIFONumber].RIR & 0x02) != 0) { /* check remote frame               */
            frm->Identifier |= STM32F20X_RTR_FRAME_BIT;/* mark remote frame                       */
        }
                                                      /* Get the DLC                              */
...
CPU_INT16S STM32F20XCANWrite (CPU_INT16S paraId, CPU_INT08U *buffer, CPU_INT16U size)
{
    ...
    if (TransmitMailbox >= 0) {
                                                      /* Set up the Id                            */
        can->TxMailBox[TransmitMailbox].TIR &= STM32F20X_CAN_TIxR_TXRQ;
        if (frm->Identifier <= 0x7FF) {
        ...
        DrvError = STM32F20X_CAN_BUSY_ERR;
    }
    CPU_CRITICAL_EXIT();
                                                      /*------------------------------------------*/
    return (result);                                  /* Return function result                   */
